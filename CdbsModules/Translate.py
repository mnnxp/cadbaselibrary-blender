...
translations_dict = {
    'ru_RU': {
        ...
        ('*', 'Add-on enables data sync with remote storage (cloud)'): 'Дополнение обеспечивает синхронизацию данных с удалённым хранилищем (облаком)',
        ...
        ('*', 'Specify a name for the new component.'): 'Укажите имя нового компонента.',
        ('*', 'Once created, the favorites list will be updated.'): 'После создания, список избранного будет обновлён.',
        ...
        ('Operator', 'Click Handler'): 'Обработка клика',
        ('*', 'Clicking on a folder will open it, while clicking on a file will attempt to load it and link it to the current collection in the scene'): 'При щелчке по папке она откроется, а при щелчке по файлу будет предпринята попытка загрузить его и связать с текущей коллекцией в сцене',
        ('Operator', 'Open directory'): 'Открыть каталог',
        ('*', "Open the folder containing the selected file or directory in system's file explorer"): 'Открыть папку, содержащую выбранный файл или каталог, в системном проводнике',
        ('Operator', 'Copy link'): 'Копировать ссылку',
        ('*', 'Copy the URL of the selected component to clipboard'): 'Копировать URL выбранного компонента в буфер обмена',
        ...
        ('*', 'You can change the synchronization process by checking the boxes below.'): 'Вы можете изменить процесс синхронизации, установив флажки ниже.',
        ...
        ('*', 'Skip comparison allows you to bypass uploading files that already exist in remote storage.'): 'Пропуск сравнения позволяет обойти загрузку файлов, которые уже существуют в удаленном хранилище.',
        ('*', 'Forced update uploads files to remote storage without performing any checks.'): 'Принудительное обновление загружает файлы в удаленное хранилище без выполнения каких-либо проверок.',
        ('*', 'Auto pull data'): 'Автоматическое получение данных',
        ('*', 'Automatically fetch (download) data from a remote source if the folder is empty.'): 'Автоматически получать (скачивать) данные из удалённого источника, если открывается пустая папка.',
        ('*', "Note: Don't enable if the connection to the remote storage (cloud) is slow."): 'Примечание: не рекомендуется, если соединение с удалённым хранилищем (облаком) медленное.',
